th.media.dt<-function(mi0,media,var,n,alpha=0.05,tipo="B",graf=TRUE){
  tc = (media-mi0)/sqrt(var/n)
 if(!graf) graphics.off()
  ...
  if(tipo=="B"){
    ...
    #regiao rejeicao
    #rrx<-seq(a[1],5,by=.1)     #limite superior =5
    #rry<-numeric(2*length(rrx))
    #rry[1:length(rrx)]<-max(rrx)
    #rrx<-c(rrx,rev(rrx))
    #polygon(rrx,rry,col="lightgray")
    #polygon(-rrx,rry,col="lightgray")
