am.ae.unif <- function(k, n)
{
  ...
  if(falta > 0)
  {
    # Como todos os restos são iguais, a distribuição
    # do restante é feita sequencialmente entre os estratos.
    ni.int[seq_len(falta)] <-
      ni.int[seq_len(falta)] + 1
  }
  ...
  # Saída
  cat("-------------------------------------------\n")
  cat("     AMOSTRAGEM ESTRATIFICADA UNIFORME\n")
  cat("-------------------------------------------\n\n")
