prob.pois.entre <- function(a, b, lamb,graf=TRUE) {
if(!graf) graphics.off()
  if(a<b & a>=0 & floor(a)==a &floor(b) == b){
    probabilidade_entre <- ppois(b, lamb)-ppois(a-1, lamb)
    ...
    cat("     ------------------------------\n")
    cat("      P(",a,"\u2264X\u2264",b,")",round(probabilidade_entre,4), "  com \u03BB = ", lamb, sep=c("","","",""," = "),fill=TRUE)
    cat("     ------------------------------\n")
  }
  else{cat("\n-----------------------\n")
